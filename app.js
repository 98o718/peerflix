...
var argv = rc(
  ...
  optimist
    ...
    .boolean('g')
    .describe('iina', 'autoplay in IINA (experimental)*')
    .boolean('boolean')
...
var ontorrent = function(torrent) {
  ...
  engine.server.on('listening', function() {
    ...
    if (argv.iina) {
      ...
      var iina = proc.exec(IINA_EXEC + iinaPlaylist);
      iina.on('exit', function() {
        fs.unlink(iinaPlaylist, err => {
          if (err) throw err;
        });
        if (!argv.n && argv.quit !== false) process.exit(0);
      });
